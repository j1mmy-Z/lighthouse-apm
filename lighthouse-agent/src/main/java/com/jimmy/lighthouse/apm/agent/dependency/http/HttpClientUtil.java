...
public class HttpClientUtil {
    ...
    static {
        ...
        connectionManager.setDefaultMaxPerRoute(5);  // 这个必须设置，默认2，也就是单个路由最大并发数是2
...
//        HttpRequestRetryHandler retryHandler = new StandardHttpRequestRetryHandler();
    ...
    public static JSONObject doHttpGet(String uri, Map<String, String> getParams) {
        ...
        try {
            ...
            log.error("CloseableHttpClient do GET error,uri={}", uri, e);
        } finally {
    ...
    public static JSONObject doHttpPost(String uri, Map<String, String> getParams) {
        ...
        try {
            ...
            log.error("CloseableHttpClient do POST error,uri={}", uri, e);
        } finally {
    ...
    private static void release(CloseableHttpResponse response, HttpRequestBase httpRequest) {
        try {
            if (null != response)
                response.close();
        } catch (IOException e) {
            log.error("CloseableHttpClient close response error", e);
        }
        try {
            if (null != httpRequest)
                httpRequest.releaseConnection();
        } catch (Exception e) {
            log.error("CloseableHttpClient release connection error", e);
        }
    }
}